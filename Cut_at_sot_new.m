...
for k=1:length(Files)
    ...
    rand_before = [randi([5,10]), randi([15,20]),randi([25,30])];
    rand_after = [0, randi([20,25]), randi([30,35])];
    % Chekc if outside range
%     if sot - ran < 0
%        ran = sot; 
%     end
    rand_before(sot - rand_before < 0) = sot;
    rand_after (sot + rand_after > size(d,1)/fs) = sot + 5;
    
    FsAfterSOT = (sot+rand_after).*fs;
...
%     DataBeforeSOT = d(FsBeforeSOT:FsBeforeSOT+time*fs,selected_channels);
    ...
    cut_soz = [soz; soz+size(d,2); soz+2*size(d,2)];
